...
> **Overtone / Supriya users:** The `thisProcess.addOSCRecvFunc` approach above is specific to sclang. OSC handling varies by implementation in Overtone and Supriya. Refer to each project's documentation for the appropriate OSC receive patterns.
...
2. *(Performer only)* Boot the SC server and set up the remote proxy receive function in your editor first.
...
5. **Name field:**
   - **Performer mode** (scsynth already running): enter your name. Must be unique in the room, must not be empty, must not exceed 64 characters, and must not contain `/`.
