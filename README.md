...
If `--name` or `--room` are omitted, you will be prompted for them on startup. Each participant's name must be unique within a room, must not be empty, and must not contain `/` — if the name is already in use or invalid the hub will reject the connection.
...
5. **Name field:**
   - **Performer mode** (scsynth already running): enter your name. Must be unique in the room, must not be empty, and must not contain `/`.
