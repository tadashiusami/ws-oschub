...
ผู้แสดงแต่ละคนต้องมีฟังก์ชันรับต่อไปนี้ทำงานใน SC ก่อนเซสชันจะเริ่ม โดยรับ OSC จากผู้แสดงคนอื่น (ส่งมาจาก local.py ไปยังพอร์ต 57120) ลบ prefix `/remote/<n>/` ที่ฮับเพิ่มเข้ามา และส่งต่อไปยัง scsynth โดยคงไว้ซึ่ง timetag เพื่อให้ timing ของ `sendBundle` ถูกต้อง
...
|---------|------|---------|
| scsynth **ไม่ได้** ทำงาน | **ผู้ฟัง** | เปิด sclang เพื่อบูต scsynth จากนั้นส่งต่อ hub OSC ไปยัง sclang (พอร์ต 57120) ฟังก์ชันรับที่ตั้งค่าโดยอัตโนมัติจะลบ prefix `/remote/<n>/` และรีเลย์คำสั่งไปยัง scsynth โดยคงไว้ซึ่ง timetag |
...
2. รัน function ต่อไปนี้ในตัวแก้ไขของคุณ:
...
> **ผู้ใช้ Overtone / Supriya:** วิธี `thisProcess.addOSCRecvFunc` ข้างต้นใช้เฉพาะกับ sclang การจัดการ OSC แตกต่างกันใน Overtone และ Supriya โปรดดูเอกสารของแต่ละโปรเจกต์สำหรับรูปแบบการรับ OSC ที่เหมาะสม
...
2. *(สำหรับผู้แสดงเท่านั้น)* บูตเซิร์ฟเวอร์ SC และตั้งค่าฟังก์ชันรับ remote proxy ในตัวแก้ไขก่อน
3. เปิด Radio SCOSC
